...
public class GC1_AddCardDetails  extends SuperTestScript 
{
	MongoDBUtilMorphia mongoutil = new MongoDBUtilMorphia();
	Datastore ds = mongoutil.getMorphiaDatastoreForNlob();
	Email email = new Email();
	Membership member=new Membership();
	public GC1_AddCardDetails()
	{
		loginRequired=false;
		logoutRequired=false;
	}
	
	@Test(enabled=true, priority=340, groups={"GiftCardsRuleMailerPositive" , "All"})
	public void gC1_AddCardDetailsTC100101() throws InterruptedException
	{
		log.info("Inside Gift card 1 script");
		
		log.info("Fetching data from databse MySQL");
		InformationFromBackend info = new InformationFromBackend();
		info.getDataForCustomerInfo("GC1_AddCardDetails");  //201007
		...
		try 
		{
			...
			for (Email email : emailList) 
			{
				...
				String expectedSubject = "Orderbekräftelse: Presentkort";
				...
				String expectedTriggerName = "Giftcard: Order confirmation";
	...
	@Test(enabled=true, priority=341, groups={"GiftCardsRuleMailerNegative" , "All"})
	public void gC1_AddCardDetails() throws InterruptedException
	{
		log.info("Inside Gift card 1 script");
